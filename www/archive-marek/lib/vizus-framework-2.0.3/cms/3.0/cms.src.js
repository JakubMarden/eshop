...
{
	vizus.cms.dialog.open(url, width, height, {autoSize: false, autoResize: true, autoVisible: true, overflowX: 'auto', overflowY: 'auto'});
};
...
vizus.cms.form.itemList.select = function(id, itemId, itemName, limit)
{
	...
	if (!limit || select.options.length < limit)
	{
		...
		document.getElementById(id + 'Order').appendChild(option);
...
{
	$(this.content.window.document).find('body > img, embed, object').css({width: this.width + 'px', height: this.height + 'px', display: 'block'});
};
...
vizus.cms.archive.Uploader = function(id, url, callback, messages)
{
	...
	this.$element.append('<span class="message"></span><span class="complete"></span><span class="progress"></span>');
...
vizus.cms.archive.Uploader.prototype.state = function(status)
{
	...
	switch (status)
	{
		case 'wait': { draggable(); break; }
		case 'dragover': { break; }
		case 'uploading': { nondraggable(); break; }
		case 'processing': { nondraggable(); break; }
		case 'done': { draggable(); break; }
		case 'error': { draggable(); break; }
	}
...
vizus.cms.archive.Uploader.prototype.error = function(message)
{
	...
	window.setTimeout(function() { this.state('wait'); }.bind(this), 5000);
};
...
vizus.cms.archive.Uploader.prototype.onload = function(event)
{
	...
	this.callback && this.callback(this.id, response);
};
