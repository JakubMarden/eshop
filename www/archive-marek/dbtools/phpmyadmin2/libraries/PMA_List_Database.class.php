...
/*public*/ class PMA_List_Database extends PMA_List
{
    ...
    function getHtmlSelectGrouped($selected = '')
    {
        if (true === $selected) {
			...
			$dbs = array_keys($this->getGroupedDetails());

?>
<!-- SELECT DB BEGIN -->
<TABLE cellSpacing="0" cellPadding="0" border="0">
<TR>
 <TD><input type="text" value="" id="AcDBSearch" onKeyUp="AutocompleteDB(this)" autocomplete="off" style="width: 100px">&nbsp;</TD>
 <TD noWrap id="AcDBName" style="font: 8pt verdana"></TD>
</TR>
</TABLE>
<SCRIPT language="JavaScript">
var AcDBList = new Array(<?= "'".join("','", $dbs)."'" ?>);
var AcDB = <?= "'".@$selected."'" ?>;
var AcDBName = document.getElementById('AcDBName');
var AcDBSearch = document.getElementById('AcDBSearch');
function AutocompleteDB(elm)
{
	var AcDBSelect = document.getElementById('lightm_db');
	if (!document.all)
		return;
	if (window.event.keyCode == 13 && AcDB)
	{
		AcDBSelect.value = AcDB;
		AcDBSelect.form.submit();
		return;
	}
	if (!elm.value.length)
		return;
	for (var i = 0; i < AcDBList.length; i++)
	{
		if (AcDBList[i].indexOf(elm.value) == 0)
		{
			AcDBName.innerText = AcDB = AcDBList[i];
			return;
		}
	}
}
document.body.onload = new Function('if (AcDB) {AcDBName.innerText = AcDB;} AcDBSearch.focus();');
</SCRIPT>
<!-- SELECT DB END -->
<?
